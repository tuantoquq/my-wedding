...
export default function StoryLine({ image, title, description, year, index }: Props) {
    ...
    return (
        <div className={`z-10 w-full p-5 sm:p-10 flex flex-col md:${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'} gap-5 relative items-center shadow-[0_0px_80px_-20px_rgba(0,0,0,0.2)] justify-between relative`}>
            ...
            <div className='w-14 h-7 flex justify-center items-center rounded-md bg-cs-green-700'>
                <Typography className='font-medium text-description' variant='desc'>{year}</Typography>
            </div>

            <div className='basis-full md:basis-5/12'>
                <Typography className='font-medium pb-5' variant='h3'>{title}</Typography>
