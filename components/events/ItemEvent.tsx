...
export default function ItemEvent({ item: { image, title, time, date, location, mapUrl } }: Props) {

    return (
        <div className={`basis-full sm:basis-1/2 lg:basis-1/3 px-5 mb-10 `}>
                ...
                <div className='w-full shrink-0 flex justify-center relative'>
                    ...
                    <div className='w-[85%] flex justify-end aspect-square border-2 border-cs-green-500 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'>
                        <div className='bg-cs-green-500/90 w-min h-min px-3 py-2'>
                            <Typography variant='desc' className='font-medium'>{date}</Typography>
                        </div>
                    </div>
                </div>
                <div className='flex flex-col gap-2 justify-center items-center'>
                    <Typography className='font-medium mb-3' variant='h3'>{title}</Typography>
                    <div className='flex flex-row gap-2 items-center justify-center'>
                        <FaClock className='text-description' />
                        <Typography variant='desc' className='md:!text-sm'>{time}</Typography>
                    </div>
                    <div className='flex flex-row gap-2 items-center justify-center'>
                        <FaLocationDot className='text-description' />
                        <Typography variant='desc' className='md:!text-sm'>{location}</Typography>
                    </div>
